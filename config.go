...
func LoadEnv() {
	...
	if err := godotenv.Load(PathToCommon(GetCurrentPath() + "/.env")); err != nil {
	...
	//获取当前系统路径,读取.env文件的值
	viper.SetDefault("ROOT", PathToCommon(GetCurrentPath()))
	...
	viper.Set("CONFIG_FILE_PATH", PathToCommon(configFilePath))
